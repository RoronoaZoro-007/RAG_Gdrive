...
class UnifiedRetriever:
    ...
    def retrieve_relevant_chunks(self, query, k_pdfs=DEFAULT_K_PDFS, k_sheets=DEFAULT_K_SHEETS, chat_history=None):
        ...
        """
        try:
            print(f"🔍 [UNIFIED_RETRIEVER] Starting chunk retrieval for query: '{query[:50]}...'")
            ...
            if self.pdf_vectorstore:
                try:
                    print(f"🔍 PDF vectorstore available: {type(self.pdf_vectorstore)}")
                    ...
                    if not hasattr(self, 'opensearch_client') or not self.opensearch_client:
                        print("❌ OpenSearch client not available, falling back to simple search")
                        ...
                        print(f"📄 Retrieved {len(pdf_chunks)} PDF chunks using fallback search")
                        return pdf_chunks, sheets_chunks
                    
                    print(f"✅ OpenSearch client available: {type(self.opensearch_client)}")
            ...
            if self.sheets_index:
                try:
                    print(f"📊 Sheets index available: {type(self.sheets_index)}")
            ...
            if len(pdf_chunks) == 0 and len(sheets_chunks) == 0:
                print("⚠️ No chunks retrieved, trying simple fallback search...")
                ...
                if self.pdf_vectorstore:
                    try:
                        ...
                        for doc in pdf_docs:
                            ...
                            pdf_chunks.append(chunk)
                        print(f"📄 Fallback: Retrieved {len(pdf_chunks)} PDF chunks")
                    except Exception as e:
                        print(f"❌ Fallback PDF search failed: {str(e)}")
                    ...
                    try:
                        ...
                        if hasattr(sheets_response, 'source_nodes'):
                            for node in sheets_response.source_nodes:
                                ...
                                sheets_chunks.append(chunk)
                        print(f"📊 Fallback: Retrieved {len(sheets_chunks)} sheet chunks")
                    except Exception as e:
                        print(f"❌ Fallback sheet search failed: {str(e)}")
            
            print(f"📊 Final result: {len(pdf_chunks)} PDF chunks, {len(sheets_chunks)} sheet chunks")
            return pdf_chunks, sheets_chunks
